        ...
        // دالة إشعار مؤقت واضح (توست) مع زر إغلاق — أي نقرة على الرسالة تغلقها فوراً
        function showToast(msg) {
            var el = document.createElement('div');
            el.style.cssText = 'position:fixed;top:16px;left:50%;transform:translateX(-50%);background:#1a1a2e;color:#fff;padding:0;border-radius:14px;font-size:13px;line-height:1.6;font-weight:bold;z-index:99999;direction:rtl;box-shadow:0 12px 40px rgba(0,0,0,0.35);display:flex;align-items:stretch;gap:12px;max-width:min(92vw,460px);cursor:pointer;border:1px solid rgba(255,255,255,0.15)';
...
function escJs(s) { return JSON.stringify(String(s == null ? '' : s)); }
        ...
        function showOrderSuccess(cartItems, d) {
            ...
            ptsEl.innerHTML = '⭐ نقطة واحدة ستُضاف إلى رصيدك بعد تأكيد المزود للطلب';
            ...
            if (d.vendors && d.vendors.length) {
                d.vendors.forEach(function(v, idx) {
                    ...
                    var discountPerItem = d.discount_amount ? (d.discount_amount / cartItems.length) : 0;
                    ...
                    var finalTotal = d.discount_amount > 0 ? (totalAmount - (d.discount_amount * vendorItems.length / cartItems.length)).toFixed(2) : totalAmount.toFixed(2);
                    var msg = 'السلام عليكم 👋\nطلب جديد من متجر ماتريكس برو\n\n👤 اسم العميل: ' + document.getElementById('checkout-name').value.trim() + '\n📱 رقم الهاتف: ' + document.getElementById('checkout-phone').value.trim() + '\n\n🛒 المشتريات:\n' + itemsList + '\n\n💰 المجموع الكلي: ' + finalTotal + ' د.ل' + '\n\n✅ بعد تأكيدك للطلب سيحصل العميل على نقاطه.\n\nأرجو تأكيد الطلب وشكراً.';
        ...
        function showSubDetail(sub, vendorName) {
            currentSubDetail = { subscription_id: sub.id, vendor_id: sub.vendor_id, subscription_name: sub.name, amount: sub.price, duration: sub.duration || '', vendor_display: vendorName || '' };
            ...
            if (sub.image_path) { img.innerHTML = '<img src="' + escAttr(sub.image_path) + '" class="w-full h-21 object-contain bg-deep-50">'; }
                ...
                allOfferings.forEach(function(o) {
                    ...
                    cats.forEach(function(c) {
                        ...
                        if (c.subscriptions) {
                            c.subscriptions.forEach(function(s) {
                                ...
                                var subData = JSON.stringify({id: s.id, vendor_id: v.id, name: s.name, price: effectivePrice, duration: s.duration || '', description: s.description || '', image_path: s.image_path || '', is_paused: s.is_paused}).replace(/"/g, '&quot;');
                    ...
                    if (o.uncategorized && o.uncategorized.length) {
                        ...
                        o.uncategorized.forEach(function(s) {
                            ...
                            var subData2 = JSON.stringify({id: s.id, vendor_id: v.id, name: s.name, price: effectivePrice2, duration: s.duration || '', description: s.description || '', image_path: s.image_path || '', is_paused: s.is_paused}).replace(/"/g, '&quot;');
