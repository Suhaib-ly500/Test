...
const { turso, ensureSchema } = require('./src/infrastructure/persistence/db');

const app = express();

// خلف بروكسي (Render/Zeabur/Koyeb) ليعمل express-rate-limit على X-Forwarded-For بشكل صحيح
...
// Serve uploaded images from the database (persists on ephemeral hosting like Render)
...
async function start() {
  ...
  try {
    if (ADMIN_USER && ADMIN_PASS) {
      const adminExists = await vendorRepository.findAdminByRole();
      if (!adminExists) {
      ...
      console.log('لم يتم إعداد حساب المشرف بعد. افتح لإنشائه');
    }
  } catch (e) {
    console.error('فشل إنشاء حساب المشرف:', e.message);
  }
  ...
  app.use((err, req, res, next) => {
    ...
    console.error('API ERROR:', req.path, String(err && err.stack || err));
  ...
  app.listen(config.port, () => {
    ...
    console.log('منصة ماتريكس برو تعمل عبر ' + proto + ' على: http://localhost:' + config.port);
